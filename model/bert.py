...
class BERTCvaeTrans(nn.Module):
    ...
    def __init__(self, vocab, model_file_path=None, is_eval=False, load_optim=False):
        super(BERTCvaeTrans, self).__init__()
        # Use pre-trained BERT base, with dropout rate 0.4
        self.bert = transformers.BertModel.from_pretrained("bert-case-cased")
        self.bert_drop = nn.Dropout(0.4)
        # Number of hidden layers of bert base is bert_hidden_dim (768 by default)
        ...
        if config.multitask:
            ...
            self.emo = SoftmaxOutputLayer(config.hidden_dim)
