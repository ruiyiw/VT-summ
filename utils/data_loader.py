...
class Dataset(data.Dataset):
    ...
    def preprocess(self, arr, anw=False, meta=None, posterior=False):
        ...
        if(anw):
            ...
            # CLS token is special for transformer
...
def collate_fn(data):
    """
    data: is a list of tuples returned by __getitem__ function, containing "context", "context_mask", etc. 
    """
    def merge(sequences):
        """
        Doing padding for sequences.
        """
        lengths = [len(seq) for seq in sequences]
        padded_seqs = torch.ones(len(sequences), max(lengths)).long() ## padding index 1
        for i, seq in enumerate(sequences):
            end = lengths[i]
            padded_seqs[i, :end] = seq[:end] ## padding at the beginning
...
def prepare_data_seq(batch_size=32):  
    ...
    logging.info("Vocab size: {} ".format(vocab.n_words))
    logging.info("tra size: {} dev size: {} tst size: {}".format(len(pairs_tra), len(pairs_val), len(pairs_tst)))
