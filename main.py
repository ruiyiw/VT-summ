...
if(config.test):
    ...
    loss_test, ppl_test, kld_test, bow_test, elbo_test, bleu_score_g, d1,d2,d3 = evaluate(model, data_loader_tst ,ty="test", max_dec_step=50)
    logging.info(loss_test, ppl_test, kld_test, bow_test, elbo_test, bleu_score_g, d1,d2,d3)
    exit(0)
...
check_iter = 1000
iter_per_epoch = math.ceil(data_loader_tra.__len__() / config.batch_size)
logging.info("iter_per_epoch: {}".format(iter_per_epoch))
...
try:
    ...
    for n_iter in tqdm(range(1000000)):
        ...
        if (n_iter+1) % iter_per_epoch == 0:
            ...
            model.save_model_by_epoch(epoch, best_elbo,n_iter,ppl_val ,0,bleu_score_g,kld_val)
            weights_best = deepcopy(model.state_dict())
            ...
            #         # if n_iter > 600000: break # batch_size=16
            #         # if n_iter > 300000: break # batch_size=32
            #         if n_iter > 100000: break # batch_size=64
            #         #if n_iter > 40000: break # batch_size=128
